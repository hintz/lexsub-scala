...
object CreateCandidateList extends App {
  ...
  val expansions = for(Token(word, pos, lemma) <- targets;
                       (expanded, relation) <- mwn.lookup(lemma, pos, RelationDepth)) yield Seq(lemma, pos, expanded, relation).mkString("\t")
                       
                       
  io.write(Settings.resourcesFolder + "/candidates/mwe.tsv", expansions.mkString("\n"))  
